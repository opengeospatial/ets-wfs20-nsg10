...
public class NsgWfsAssertion {
	...
	/**
	 * Validates if the passed id is not null and a valid UUID or GUIDE ID identifier. The
	 * format of the GUIDE id is as follows: guide://{prefix}/{suffix}
	 ...
	 * @param message used as assertion message
	 */
	...
	/**
	 * Asserts if the passed WFS capabilities contains the outputFormat
	 * 'application/gml+xml; version=3.2' for the specified operation.
	 * @param wfsMetadata the document to parse, never <code>null</code>
	 * @param operation the operation to parse the outputFormat for, never
	 * <code>null</code>
	 * @throws javax.xml.xpath.XPathExpressionException if an expression could not be
	 * evaluated (should never happen)
	 */
	public static void assertOutputFormat(Document wfsMetadata, String operation) throws XPathExpressionException {
		List<String> outputFormats = parseParameters(wfsMetadata, operation, "outputFormat");

		String msg = String.format(
				"The operation %s is not supported or does not support the outputFormat application/gml+xml; version=3.2",
				operation);
	 ...
	 * @param wfsMetadata the capabilities, never <code>null</code>
