...
public class DescribeFeatureTypeFixture extends BaseFixture {
    ...
    public void createRequestEntity() {
        try {
            this.reqEntity = docBuilder.parse( getClass().getResourceAsStream( "/org/opengis/cite/iso19142/simple/DescribeFeatureType.xml" ) );
        } catch ( Exception e ) {
            TestSuiteLogger.log( Level.WARNING, "Failed to parse request entity from classpath", e );
        }
    }
    ...
    /**
     * Adds a wfs:TypeName child element to a wfs:DescribeFeatureType entity. A suitable namespace binding will be added
     * to the document element if necessary.
     *
     * @param qName
